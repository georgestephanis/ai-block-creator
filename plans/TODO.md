...
- [ ] Add PHPUnit tests for `render_template()` using the cases in the review (triple-brace, nested if, list + outer var, attribute context, booleans, arrays) — a WP PHPUnit test scaffold (`tests/bootstrap.php`, `.wp-env.json` or similar) doesn't exist yet; the fixes were verified ad hoc against a live `wp-load.php` instead (see commit `518e433`'s message). [DX-5]
- [ ] Add Jest tests for `interpolateTemplate()` asserting parity with the PHP fixtures. `@wordpress/scripts test-unit-js` is available but not yet wired up. [DX-5]
...
- [ ] `Requires at least: 7.0` — done in the plugin header [DX-1] `518e433`; **`readme.txt` and `README.md` still say 6.7/7.0+ inconsistently and need a pass**. Consider `Requires Plugins:` or an admin notice when no connector is registered.
...
- [ ] CI: `npm ci && npm run build` and fail on `git diff --exit-code build/`; run `composer lint` (phpcs), `npm run lint-js`, `npm run lint-css`, plus the PHPUnit/Jest suites once DX-5's tests exist. [DX-4]
...
- [x] **Set up JS/CSS lint + format via `@wordpress/scripts`** — `npm run lint-js[:fix]`, `npm run lint-css[:fix]`, `npm run format`, `npm test` (runs both linters). All `src/` files pass clean. Also had to pin `typescript` to `5.6.3` via npm `overrides` — the resolved `typescript@7` broke `@typescript-eslint@6`'s `ts-api-utils`, crashing ESLint outright; worth revisiting once `@wordpress/eslint-plugin` bumps its typescript-eslint dependency. [DX-5] `e8240c6`
- [ ] PHPUnit + Jest test suites themselves are still not scaffolded (the lint *tooling* is done, but there's no `tests/` directory, no `.wp-env.json`/PHPUnit bootstrap, no Jest config beyond what `@wordpress/scripts test-unit-js` provides out of the box). [DX-5]
...
- [ ] Update `AGENTS.md` — rule #1 now matches the code (uses `wp_ai_client_prompt()`), but the doc itself hasn't been edited to reflect the `AI_Block_Store` architecture, the capability split, or the renamed CPT. Update `architecture-and-design.md` §2.1 to reflect the real capability model. [DX-9]
- [ ] `README.md`/`readme.txt` describe the pre-fix architecture in places (e.g. don't mention `AI_Block_Store`, the capability split, or the CPT rename) — needs a pass once the remaining P2/P3 items settle, to avoid documenting churn twice.
